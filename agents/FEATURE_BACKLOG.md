...
|---|-----------|------------------------------|--------|
| **A-1** | **Approvals + Ledger engine** (`apps.approvals`: ApprovalRequest + LedgerEntry): `request → approve/reject → cashier disburses → immutable ledger row` | Expenses, staff loans, procurement (#15), payment-delay (#5), discount requests (#5/#7), partial-pay, salary-prep (#7), event cost-split (#14), book cash-sales (#8), rewards/points payouts (#6/#7) — ALL one engine. The ledger is the anti-fraud moat ("money can't disappear"). | **CORE DONE** (`/api/v1/approvals/requests/` + `/ledger/`). TODO: notify-on-disburse, multi-step approval chains, fold in Expenses, build loans/procurement as kinds. |

...
| # | Feature | Acceptance | New/Reuse | Status |
|---|---------|-----------|-----------|--------|
| F8-1 | Dynamic answer types: multiple-choice, true/false (default), writing, reading, listening, speaking, vocabulary | per-question type; manager enables types | extend D-4 | PARTIAL — single_choice / true_false / writing + **multiple_choice (multi-select)** done: `MULTIPLE_CHOICE` type, answer key is a non-empty unique subset of options, all-or-nothing set grading (order-independent, no partial credit), counts through the unified `_grade_attempt`; full validation (never-raise on AI junk, unhashable-guarded). TODO: reading/listening/speaking (need media), vocabulary (matching), manager-enables-types policy. |

...
- **TD-18 — API versioning & errors.** Everything under `/api/v1/`; the error envelope `{"error": {"code", "detail", "fields?"}}` from `core/exceptions.py` is universal — webhook endpoints included. Breaking changes post-handoff require `/api/v2/`, not mutation.
